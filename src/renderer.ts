...
export class Renderer {
    ...
    async serializeFx(requestUrl: string, isMobile: boolean):
        Promise<SerializedFxResponse> {
        ...
        if (!response) {
            ...
            // https://github.com/GoogleChrome/puppeteer/blob/v1.5.0/docs/api.md#pagegotourl-options.
            await page.close();
            return {status: 400, content: '', url: '', title: ''};
        }
        ...
        if (response.headers()['metadata-flavor'] === 'Google') {
            await page.close();
            return {status: 403, content: '', url: '', title: ''};
        }
        ...
        const fxUrl = await page.evaluate('window.location.href');
        const fxTitle = await page.evaluate('document.title');
        const result = await page.evaluate('document.firstElementChild.outerHTML');

        const webdriver = await page.evaluate('navigator.plugins.length');
        console.info(webdriver);

        await page.close();

        return {status: statusCode, content: result, url: fxUrl, title: fxTitle};
    }
